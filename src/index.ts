...
async function main(): Promise<void> {
  ...
  const client = new AdvancedHttpClient({ baseURL: 'http://localhost:5003/api/v1' });
  const p1 = client.get('test?status=500&statusAfter=200&per=4');
  ...
  const { data: d3 } = await client.get('test?status=500&statusAfter=200&per=4');
  console.log(d3);
}
